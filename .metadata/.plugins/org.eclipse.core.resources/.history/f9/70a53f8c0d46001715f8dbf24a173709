...
public class ItemTicketDao {
	...
	public int agregarItemTicket(ItemTicket objeto) {
		int id = 0;
		try {
			iniciaOperacion();
	...
	public void eliminarItemTicket(ItemTicket objeto) throws HibernateException {
		try {
			iniciaOperacion();
	...
	public ItemTicket traerItemTicket(long idItemTicket) throws HibernateException {
	...
	public List<ItemTicket> traerItemTicket() throws HibernateException {
		...
		try {
			iniciaOperacion();
			lista= session.createQuery("from ItemTicket i order by i.idItemTicket asc").list() ;
		} finally {
			session .close();
		}
		return lista;
